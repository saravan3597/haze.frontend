...
export const generateAIWallpaper = functions
  ...
  if (!context.auth) {
    throw new functions.https.HttpsError(
      'unauthenticated',
      'Must be signed in to generate AI wallpapers.',
    );
  }
  ...
  if (context.auth.token.firebase.sign_in_provider === 'anonymous') {
    throw new functions.https.HttpsError(
      'permission-denied',
      'Sign in to use AI wallpaper generation.',
    );
  }
  ...
  let usageSnap: FirebaseFirestore.DocumentSnapshot;
  try {
  ...
  const currentCount: number = usage['date'] === today ? (usage['count'] as number) : 0;
